...
public class ProcessInstanceDetailPanel extends DetailPanel {
          ...
          if (!bpmnModel.getFlowLocationMap().isEmpty()) {
            ...
            URL url = new URL(explorerURL.getProtocol(), explorerURL.getHost(), explorerURL.getPort(),
                explorerURL.getPath().replace("/ui", "") + "diagram-viewer/index.html?processDefinitionId="
                    + URLEncoder.encode(processDefinition.getId(), "UTF-8") + "&processInstanceId="
                    + URLEncoder.encode(processInstance.getId(), "UTF-8"));
  ...
  protected void addTasks() {
    ...
    if (!tasks.isEmpty()) {
      ...
      taskTable.addContainerProperty("name", String.class, null, i18nManager.getMessage(Messages.TASK_NAME), null,
          Table.ALIGN_LEFT);
